...
export type PackApplyOptions = {
  ...
  promptInstallAgenticStackPython?: () => Promise<boolean>;
};

const homebrewBinPaths = ['/opt/homebrew/bin', '/usr/local/bin', '/home/linuxbrew/.linuxbrew/bin'];
...
export const realPackRunner: PackCommandRunner = {
  async run(cwd, command, args) {
    const result = await execa(command, args, {
      ...
      stdio: 'pipe',
      env: {
        PATH: [...homebrewBinPaths, process.env.PATH ?? ''].join(delimiter),
      },
    });
...
function isAgenticStackPythonCompatibilityError(error: unknown): boolean {
  const message = errorMessage(error);
  return message.includes("unsupported operand type(s) for |") && message.includes('types.GenericAlias');
}
...
async function installHomebrewPython(target: string, runner: PackCommandRunner): Promise<void> {
  await runner.run(target, 'brew', ['install', 'python']);
}
...
export async function applyDevelopmentPacks(
  ...
  try {
    ...
    if (isAgenticStackPythonCompatibilityError(error)) {
      const platform = applyOptions.platform ?? process.platform;
      if (platform === 'win32') {
        return [
          {
            ...
            status: 'failed',
            message:
              'Agentic Stack is installed but its Python runtime is not compatible. Install Python 3.10+ and rerun this command.',
          },
        ];
      }
        ...
        return [
          {
            ...
            status: 'failed',
            message:
              'Agentic Stack is installed but is running with an unsupported Python version. Install Python 3.10+ and make sure python3 resolves to it, then rerun this command.',
          },
        ];
      }

      const shouldInstall = applyOptions.promptInstallAgenticStackPython
        ? await applyOptions.promptInstallAgenticStackPython()
        : false;
      if (shouldInstall) {
        try {
          await installHomebrewPython(options.target, runner);
          await runner.run(options.target, 'agentic-stack', ['codex', '--yes']);
          return [
            {
              pack: 'agentic-stack',
          ...
          return [
            {
              pack: 'agentic-stack',
              status: 'failed',
              message: `Agentic Stack Python runtime repair failed: ${errorMessage(installError)}`,
            },
          ];
        }
      }
      ...
      return [
        {
          ...
          status: 'failed',
          message:
            'Agentic Stack is installed but is running with an unsupported Python version. Install Homebrew Python or put Python 3.10+ before the system python3 in PATH, then rerun this command.',
        },
      ];
    }
